...
def get_images_from_baidu(keyword, page_num, save_dir):
    ...
    # url = 'https://cn.bing.com/images/search/acjson?'
    ...
    for pn in range(0, 30 * page_num, 30):
        ...
        for image_url in image_url_list:
            ...
            with open(os.path.join(save_dir, f'{n:010d}.jpg'), 'wb') as fp:
                fp.write(image_data)
            n = n + 1


if __name__ == '__main__':
    # keywords = ['特斯拉', '上汽大通', '昌河', '欧宝', '华泰', '五十铃', '观致', '克莱斯勒', '北京', '依维柯', '林肯', 'WEY',  '比速', '红旗','讴歌',
    #             '英致', '哈飞', '宝沃', '知豆', '福田', '玛莎拉蒂']  # 搜索关键词

    keywords = ['菜刀', '水果刀', '棒球棍', '棍子']
    root_save_dir = r'F:\workfile\gudong\20240813data'  # 图片存放地址
    for keyword in keywords:
        save_dir = os.path.join(root_save_dir, keyword)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        page_num = 100  # 爬取多少页图片
